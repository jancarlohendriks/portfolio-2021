...
export default class CursorFx {
	constructor({ el, base_class }, options = {}) {
		...
		this.DOM.circle = this.DOM.el.querySelector(`${base_class}__inner__outside`)
		...
		if (this.bounds.dot && !this.bounds.dot.width) {
			...
			this.bounds.dot.width = parseInt(
				COMPUTED_STYLES.getPropertyValue('width').replace('px', '')
			)
			this.bounds.dot.height = parseInt(
				COMPUTED_STYLES.getPropertyValue('height').replace('px', '')
			)
		}

		if (this.bounds.circle && !this.bounds.circle.width) {
			const COMPUTED_STYLES = window.getComputedStyle(this.DOM.circle)
		...
		if (this.bounds.custom && !this.bounds.custom.width) {
			const COMPUTED_STYLES = window.getComputedStyle(this.DOM.custom)
	...
	initEvents(add = true) {
		const mouseMove = (ev) => (this.mousePos = getMousePos(ev))
