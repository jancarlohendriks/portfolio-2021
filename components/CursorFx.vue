...
export default {
	...
	methods: {
		...
		initEvents() {
			...
			;[...document.querySelectorAll('[data-cursor-hover]')].forEach((link) => {
			...
			;[...document.querySelectorAll('[data-cursor-hidden]')].forEach(
				(link) => {
			...
			;[...document.querySelectorAll('[data-cursor-mix-blend-mode]')].forEach(
				(link) => {
		...
		async destroy(refresh = false) {
			...
			// Events destroy
			;[...document.querySelectorAll('[data-cursor-hover]')].forEach((link) => {
				link.removeEventListener('mouseenter', this.cursorLeave, false)
		...
		async start() {
			...
			this.$timeout = setTimeout(() => this.init(), parseInt(this.delay))
		},
		refresh() {
			this.destroy(true)
		},
